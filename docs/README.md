# Documentation

## Core Reference
- [Architecture](architecture.md) - System design, data flow, deployment
- [API Reference](api.md) - All endpoints, parameters, response formats
- [API Scopes](api/scopes.md) - Resource bundles and `access_policy` for API keys (v4.4.0+)
- [Scoring Design](scoring-design.md) - Asset-level and chain-level risk score formulas
- [Deployment](DEPLOYMENT.md) - Production deploy, migrations, post-deploy checklist

## Guides
- [Adding a Stablecoin](guides/adding-asset.md)
- [Adding a Chain](guides/adding-chain.md)
- [Plugin Development](guides/plugins.md)
- [AI Configuration](guides/ai-configuration.md) - Provider registry + 3-tier fallback
- [Alert Routing](guides/alert-routing.md) - Multi-webhook + SMTP (v4.4.0+)
- [Webhook Alerts](guides/webhook-alerts.md) - HMAC payload schema
- [Cross-tab Auth](guides/cross-tab-auth.md) - Single-operator session model

## Concepts
- [Data Methodology](concepts/data-methodology.md) - Scoring, supply velocity, regime detection, anomaly detection

## External
- [Contributing](../CONTRIBUTING.md)
- [Security](../SECURITY.md)
- [Changelog](../CHANGELOG.md)
